...
import datetime
import random
obj_db=Dal.RecurringAccountDB()

class CRecurringAccount(Account.CAccount):
    ...
    def m_deposit(self, obj_ent=Bent.AccountsEntities()):
        ...
        amount = obj_db.fetch_last_Recurring_acc_amount(obj_ent)  # calling function for fetching last transaction amount
        if amount is None:
            amount = 0.00
        self.__balance = float(amount) + float(obj_ent.getamount())
        obj_ent.set_total_amount(self.__balance)
        obj_db.Recurring_Account_transaction(obj_ent)

    def m_withdrawal(self, obj_ent=Bent.AccountsEntities()):
        try:
            obj_db.m_mssql_connect()
            amount = obj_db.fetch_last_Recurring_acc_amount(obj_ent)
    ...
    def m_insert_account_info(self, obj_ent=Bent.AccountsEntities()):
        obj_db.m_mssql_connect()
        obj_db.Recurring_Account_info(obj_ent)

    def m_fetch_all_recurring_acc_details(self, obj_ent=Bent.AccountsEntities()):
        obj_db.m_mssql_connect()
        return obj_db.fetch_all_Recurring_acc_details(obj_ent)

    def m_fetch_all_account_transactions(self, obj_ent=Bent.AccountsEntities()):
        obj_db.m_mssql_connect()
        return obj_db.fetch_all_Recurring_acc_transactions(obj_ent)
