...
import datetime
import random
obj_db=Dal.CurrentAccountDB()

class CCurrentAccount(Account.CAccount):
    ...
    def m_deposit(self, obj_ent=Bent.AccountsEntities()):
        ...
        amount = obj_db.fetch_last_current_acc_amount(obj_ent)  # calling function for fetching last transaction amount
        if amount is None:
            amount = 0.00
        self.__balance = float(amount) + float(obj_ent.getamount())
        obj_ent.set_total_amount(self.__balance)
        obj_db.Current_Account_transaction(obj_ent)

    def m_withdrawal(self, obj_ent=Bent.AccountsEntities()):
        try:
            obj_db.m_mssql_connect()
            amount = obj_db.fetch_last_current_acc_amount(obj_ent)
            if float(amount) < float(obj_ent.getamount()):
    ...
    def m_insert_account_info(self, obj_ent=Bent.AccountsEntities()):
        obj_db.m_mssql_connect()
        obj_db.Current_Account_info(obj_ent)

    def m_fetch_all_current_acc_details(self, obj_ent=Bent.AccountsEntities()):
        obj_db.m_mssql_connect()
        return obj_db.fetch_all_current_acc_details(obj_ent)

    def m_fetch_all_account_transactions(self, obj_ent=Bent.AccountsEntities()):
        obj_db.m_mssql_connect()
        return obj_db.fetch_all_current_acc_transactions(obj_ent)
